    ...
    public JSONObject ConvertOffice(Map<String, Object> parameters) {
        ...
        try {
            ...
            if ("url".equalsIgnoreCase(strInputType)) {
                ...
                fileInput = new File(strInputPath.trim());
            }
...
//                jsonWriteBack = JSONObject.fromObject(parameters.get("writeBack"));
                jsonWriteBack =  JSONUtil.parseObj(parameters.get("writeBack"));
                if ("path".equalsIgnoreCase(strWriteBackType)) {
                    strOutPutPath = jsonWriteBack.getStr("path");
...
//                            JSONObject jsonWaterMark = JSONObject.fromObject(parameters.get("waterMark"));
                            JSONObject jsonWaterMark = JSONUtil.parseObj(parameters.get("waterMark"));
                            // 水印类型：pic（图片）、text（文字）
                            String strWaterMarkType = jsonWaterMark.getStr("waterMarkType");

                            boolean blnSuccess = false;
                            // 如果需要加入图片水印
                            if ("pic".equalsIgnoreCase(strWaterMarkType)) {
                                // 获取水印图片
                                String strWaterMarkFile = System.getProperty("user.dir") + "/watermark/" + jsonWaterMark.getStr("waterMarkFile");

                                Integer intDegree = jsonWaterMark.getInt("degree");
                                String strAlpha = jsonWaterMark.getStr("alpha");
                                ...
                                String strWaterMarkText = jsonWaterMark.getStr("waterMarkText");
                                ...
                                String strFontName = jsonWaterMark.getStr("fontName");
                                Integer intFontSize = jsonWaterMark.getInt("fontSize");
                                String strFontColor = jsonWaterMark.getStr("fontColor");
                                // 水印文字颜色
                                ...
                                if ("pdf".equalsIgnoreCase(strOutputType)) {
                                    ...
                                    if (fileOfd.exists()) {
                                        ...
                                        float floatFontSize = Float.parseFloat(jsonWaterMark.getStr("fontSize"));
                ...
                if (!"path".equalsIgnoreCase(strWriteBackType)) {
                    ...
                    if ("url".equalsIgnoreCase(strWriteBackType)) {
                        String strWriteBackURL = jsonWriteBack.getStr("url");
                        ...
                        // ftp回写
                        boolean blnPassive = jsonWriteBack.getBool("passive");
                        String strFtpHost = jsonWriteBack.getStr("host");
                        int intFtpPort = jsonWriteBack.getInt("port");
                        String strFtpUserName = jsonWriteBack.getStr("username");
                        String strFtpPassWord = jsonWriteBack.getStr("password");
                        String strFtpFilePath = jsonWriteBack.getStr("filepath");
                    ...
                    String strFlag = jsonReturn.getStr("flag");
